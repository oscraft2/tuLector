...
/**
 * REGLA DURA (APK): mismo patron mount-reveal que HeaderWhatsApp.tsx -- el
 * boton flotante NO tenia este blindaje (solo el del header), y como
 * capacitor.config.ts arranca la app en /auth (fuera de HIDDEN_PREFIXES),
 * el flotante se colaba en la pantalla de login del APK. Estado inicial
 * oculto, solo se revela tras hidratar y confirmar que NO es el contenedor
 * nativo. Cinturon adicional: clase `tl-wsp` + `html.cap-native .tl-wsp`
 * en globals.css (mismo mecanismo, ahora tambien cubre este boton).
 */
export function WhatsAppFloatingButton({ config }: { config: WhatsappButtonConfig }) {
  ...
  return (
    <a
      ...
      className={`tl-wsp fixed bottom-5 ${positionClass} z-50 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] shadow-lg transition-transform hover:scale-105`}
    >
