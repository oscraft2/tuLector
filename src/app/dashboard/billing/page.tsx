...
export default async function BillingPage() {
  ...
  // Apple/Google prohiben vender contenido digital (planes, packs de escaneos)
  ...
  const checkoutInputs: Array<{ id: string; title: string; subtitle: string; badge?: string; input: BillingCheckoutInput }> = [
    { id: "pro", title: "Plan Pro", subtitle: "2.000 lecturas para equipos pequenos", badge: "Recomendado", input: { type: "plan", plan: "pro" } },
    { id: "school", title: "Plan School", subtitle: "10.000 lecturas para alta demanda", input: { type: "plan", plan: "school" } },
    { id: "pack-500", title: "Pack 500", subtitle: "Recarga puntual de lecturas", input: { type: "scan_pack", scansAdded: 500 } },
    { id: "pack-1000", title: "Pack 1.000", subtitle: "Mejor valor para temporada de pruebas", input: { type: "scan_pack", scansAdded: 1000 } },
  ];
    ...
    <>
      <PageHeader
        ...
        description={isNativeApp
          ...
          : "Plan, cuota de lecturas, compras de scans y facturacion. En Chile el pago se procesa con Flow desde un checkout seguro."}
      />
      <div className="space-y-6">
        {isNativeApp ? (
          <div className="rounded-md border border-[#e1e5ea] bg-[#f8faf9] p-5">
            ...
            <p className="mt-2 text-sm text-[#5b6472]">
              Para comprar un plan nuevo, ampliar tu cuota de lecturas o cambiar tu suscripcion, ingresa a tulector.cl desde un navegador (computador o celular).
            </p>
          </div>
        ) : (
        ...
        <DataTable columns={["Tipo", "Estado", "Monto", "Factura", "Fecha"]} rows={orders ?? []} empty="No hay compras ni facturas." renderMobileRow={(order) => (
          <article key={order.id} className="rounded-2xl border border-[#e1e5ea] bg-white p-4 shadow-sm">
            ...
            <div className="mt-3 flex items-center justify-between text-xs text-[#64748b]">
              ...
              {order.invoice_pdf_url ? <a className="font-bold text-[#07305f] underline" href={order.invoice_pdf_url}>Factura PDF</a> : <span>Sin factura PDF</span>}
            </div>
          </article>
        )} renderRow={(order) => (
