      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <div className="space-y-6">
          <section className="rounded-md border border-[#d8dde3] bg-white p-5">
            <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><h2 className="text-xl font-semibold">Perfil</h2><button className="w-full rounded-md border border-[#cfd6df] px-4 py-2 text-sm font-semibold text-[#07305f] sm:w-auto">Editar perfil</button></div>
            <div className="flex flex-col gap-4 border-b border-[#e1e5ea] pb-5 sm:flex-row sm:items-center sm:gap-5"><div className="flex h-20 w-20 items-center justify-center rounded-full bg-[#07305f] text-3xl font-semibold text-white sm:h-24 sm:w-24 sm:text-4xl">{initials}</div><div className="min-w-0"><p className="truncate text-xl font-semibold">{email.split("@")[0]}</p><p className="mt-1 break-all text-[#5b6472]">{email}</p><StatusPill>{member.role === "admin" ? "Administradora" : member.role}</StatusPill></div></div>
            <div className="mt-5 grid gap-3">
              <Field label="Correo electronico" value={email} />
              <Field label="Perfil institucion" value={countryProfile.profileName} />
              <div className="grid gap-2 md:grid-cols-[180px_1fr] items-center">
                <span className="text-sm font-semibold text-[#4b5563]">Idioma de la cuenta</span>
                <div className="rounded-md border border-[#cfd6df] p-3 bg-[#f8fafc]">
        ...
        <div className="space-y-6">
          <section className="rounded-md border border-[#d8dde3] bg-white p-5">
            <h2 className="text-xl font-semibold">Institucion</h2>
            {isOfficialSchool && (
              <p className="mt-2 text-xs text-[#5b6472] bg-blue-50 p-2.5 rounded border border-blue-100">
                ℹ️ Esta institución está vinculada a un registro oficial (RBD {school.rbd}). Los datos de nombre e identificadores geográficos están bloqueados para evitar desajustes.
              </p>
            )}
            <form action={updateSchoolSettings} className="mt-5 grid gap-3">
              ...
              <FieldInput name="rbd" label="RBD (Rol Base de Datos)" defaultValue={school.rbd ?? ""} readOnly={isOfficialSchool || !isAdmin} />
              ...
              <label className="text-sm font-semibold">
                Pais / perfil
                <select name="country_code" defaultValue={countryProfile.code} disabled={isOfficialSchool || !isAdmin} className="mt-2 w-full rounded-md border border-[#cfd6df] bg-white px-3 py-2 font-normal disabled:bg-[#f4f6f8] disabled:text-[#5b6472]">
              ...
              <button disabled={!isAdmin} className="rounded-md border border-[#07305f] px-4 py-2 text-sm font-semibold text-[#07305f] disabled:opacity-50 cursor-pointer">
                Guardar institucion
              </button>
            </form>
          </section>
...
function Field({ label, value }: { label: string; value: string }) { return <div className="grid gap-2 md:grid-cols-[180px_1fr]"><span className="text-sm font-semibold text-[#4b5563]">{label}</span><div className="rounded-md border border-[#cfd6df] px-3 py-2 text-sm bg-white">{value}</div></div>; }
function FieldInput({ name, label, defaultValue, readOnly }: { name: string; label: string; defaultValue: string; readOnly: boolean }) { return <label className="text-sm font-semibold">{label}<input name={name} defaultValue={defaultValue} readOnly={readOnly} className={`mt-2 w-full rounded-md border border-[#cfd6df] px-3 py-2 font-normal ${readOnly ? "bg-[#f4f6f8] text-[#5b6472] cursor-not-allowed" : "bg-white text-[#0b1220]"}`} /></label>; }
...
function Row({ label, value, action }: { label: string; value: string; action: string }) { return <div className="flex flex-col gap-3 py-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-medium">{label}</p><p className="mt-1 text-sm text-[#5b6472]">{value}</p></div><button className="w-full rounded-md border border-[#9aa8ba] px-4 py-2 text-sm font-semibold text-[#07305f] sm:w-auto">{action}</button></div>; }
