...
const FIELD_CLASS = "w-full rounded-md border border-[#cfd6df] bg-white px-3 py-2 outline-none focus:border-[#2563eb] focus:ring-2 focus:ring-[#2563eb]/20";
  ...
  return (
    <form action={action} className="space-y-5">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Field label="Título" hint="Claro y orientado a una tarea."><input name="title" value={title} onChange={(e) => setTitle(e.target.value)} maxLength={160} required className={FIELD_CLASS} /></Field>
        <Field label="Slug (URL)"><input name="slug" value={slug} onChange={(e) => setSlug(e.target.value)} maxLength={160} required className={FIELD_CLASS} /></Field>
      </div>
      <div className="grid gap-4 md:grid-cols-2">
        <Field label="Idioma"><select name="locale" value={locale} onChange={(e) => setLocale(e.target.value)} className={FIELD_CLASS}><option value="es-CL">es-CL</option><option value="es-MX">es-MX</option><option value="es-PE">es-PE</option><option value="es-AR">es-AR</option><option value="pt-BR">pt-BR</option></select></Field>
        <Field label="Categoría"><select name="category_id" defaultValue={article?.category_id ?? ""} required className={FIELD_CLASS}><option value="" disabled>Selecciona una categoría...</option>{matchingCategories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></Field>
      </div>
      <Field label="Resumen" hint="Se muestra en las tarjetas y en buscadores."><textarea name="excerpt" defaultValue={article?.excerpt ?? ""} maxLength={300} rows={2} className={FIELD_CLASS} /></Field>
