    ...
    <AdminShell active="/admin/schools" title="Colegios" description="Tenants multi-tenant: estado, plan, cuota, miembros y acciones con auditoria.">
      <DataTable columns={["Colegio", "Pais", "Plan", "Cuota", "Estado", "Acciones"]} rows={schools ?? []} empty="No hay tenants." renderRow={(school) => (
        <tr key={school.id} className="border-b border-[#eef0f3] last:border-0">
          ...
          <td className="px-5 py-4"><form action={updateSchoolPlan} className="flex gap-2"><input type="hidden" name="school_id" value={school.id} /><input type="hidden" name="reason" value="Ajuste manual desde admin" /><select name="plan" defaultValue={school.plan} className="rounded border border-[#cfd6df] px-2 py-1 text-xs"><option value="starter">starter</option><option value="pro">pro</option><option value="school">school</option><option value="district">district</option></select><button className="rounded border border-[#cfd6df] px-2 py-1 text-xs font-semibold">Guardar</button></form></td>
        </tr>
      )} />
    </AdminShell>
  );
