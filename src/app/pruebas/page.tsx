...
export default function PruebasPage() {
  ...
  const compare = async () => {
    ...
    try {
      ...
      setReport(buildReport(truth, logs));
    } catch (e) {
    ...
    <div className="min-h-screen bg-zinc-950 text-white">
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 space-y-5">
        <section className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 space-y-3 text-sm">
          ...
          {truthName && <p className="text-green-400 text-xs">✓ {truthName} — {truth.length} hojas</p>}
          <button onClick={compare} disabled={busy || truth.length === 0}
            ...
            {busy ? "Comparando…" : "2. Traer escaneos y comparar"}
          </button>
